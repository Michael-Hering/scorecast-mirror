...
export const TweetPanel = ({ city }: Props) => {
  ...
  const [tweetState, setTweetState] = useState({ loaded: false, city: city, tweetComponents: null })
  ...
  if (!tweetState.loaded) {
    ...
    axios.post("http://localhost:5000/api/tweets", { topic: city })
      .then((res: any) => {
        const components: any = []
        res.data.forEach((tweetRes: any) => {
          const tweet = JSON.parse(tweetRes);
          components.unshift(< TwitterTweetEmbed tweetId={tweet.id_str} key={uuid.v4()} theme="dark"/>)
        });

      // State update
      setTweetState({ loaded: true, city: city, tweetComponents: components })
    })

    // State indicated tweets are loading, show a spinner
    return (
      <DashPanel dashLocation={'twitter'} dashName={"Live Tweets"}>
        <div style = {{margin: "55% auto auto auto", width:"10%", height:"10%"}}>
